...
module.exports = {
  ...
  construct: function (self, options) {
    ...
    self.afterSave = function (req, piece, options, callback) {
      ...
      const args = [ 'apostrophe-global:build-favicons' ];
...
the global preferences are saved.`,
      ...
      function body(callback) {
        ...
        if (piece.aposFavicon.items.length !== 0 && ((piece._originalFaviconRelationship && JSON.stringify(piece._originalFaviconRelationship) !== JSON.stringify(compare) && piece.aposFavicon.items[0] !== undefined) || piece._originalFaviconRelationship === undefined)) {
          ...
          return self.apos.docs.db.update({ _id: piece._id }, { $unset: { aposFaviconProgress: true } }, callback);
        }
        ...
        function setStatusToProcessing (callback) {
          ...
          console.log('p');
          return self.apos.docs.db.update({ _id: piece._id }, { $set: { aposFaviconProgress: true } }, callback);
        }
        ...
        function setStatusToNotProcessing (callback) {
          console.log(arguments);
          console.log('not');
          return self.apos.docs.db.update({ _id: piece._id }, { $unset: { aposFaviconProgress: '' } }, callback);
        }
